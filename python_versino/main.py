...
# from node import Tag, Node, hide_dups, print_tree
...
# def dedup_tree(term:Node, ctx:dict[Node, int])->str:
#   def varname(node:Node | None):
#     if node is None: return ""
#     return ctx.setdefault(node, chr(len(ctx) + 97))
#   def idn(lns:list[str])->list[str]:
...
#       case Tag.dup | Tag.dup2:
#         return _tree(term.s0, dstack + ([(term.label, term.tag == Tag.dup2)] if hide_dups else []))
#       case Tag.Prim: return [str(term.label)]
#       case Tag.Null: return ["Nul"]
#     return [varname(term)]
#   return "\n".join(_tree(term, []))
...
# a = dup(dup(sup(
#   sup(num(2),num(3), 0), num(1), 0), 0)[0], 0)[1]
...
# print(dedup_tree(app(church_nat(2), church_nat(2)), {}))
...
void say_hello(int n) {
    for (int i = 0; i < n; i++) printf("Hello %d\\n", i);
}
"""

# 2. Write C code to a temp file
with tempfile.TemporaryDirectory() as tmp:
    c_path = os.path.join(tmp, "hello.c")
    so_path = os.path.join(tmp, "hello.so")

    with open(c_path, "w") as f:
        f.write(code)

    # 3. Compile to shared library using clang
    ...
    # 5. Call the function
    lib.say_hello.argtypes = [ctypes.c_int]
    lib.say_hello(3)
